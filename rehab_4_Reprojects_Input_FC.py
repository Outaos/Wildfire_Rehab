...
def reproject_shapefiles_in_group(fire_number):
    ...
    if not group_layer:
        arcpy.AddError(f"Group layer '{group_layer_name}' not found.")
        return
        ...
        if lyr.dataSource.endswith(".shp"):
            ...
            if desc.spatialReference.factoryCode == 4326:
                ...
                if arcpy.ValidateTableName(output_name, default_gdb) != output_name:
                    arcpy.AddWarning(f"Adjusted output name '{output_name}' is invalid in the geodatabase.")
                ...
                arcpy.AddMessage(f"Reprojected and added '{output_name}' to group '{group_layer_name}'.")
            else:
                arcpy.AddMessage(f"'{lyr.name}' is not in WGS 1984, skipping.")
    
    # Save the project to retain changes
    aprx.save()

    arcpy.AddMessage("Reprojection completed for all eligible shapefiles.")

# User provides only the fire number as a parameter
fire_number = arcpy.GetParameterAsText(0)

# Run the function with the provided fire number
reproject_shapefiles_in_group(fire_number)